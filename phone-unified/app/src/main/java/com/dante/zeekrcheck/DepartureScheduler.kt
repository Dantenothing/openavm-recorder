...
object DepartureScheduler {
    ...
    private fun pending(context: Context, id: String, occurrence: Long = 0) = PendingIntent.getBroadcast(context, 0,
        Intent(context, DepartureReceiver::class.java).setAction("com.dante.zeekrcheck.DEPARTURE.$id")
            .putExtra("planId", id).putExtra("occurrence", occurrence), PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
    ...
    fun sync(context: Context) {
        ...
        state.plans.forEach { plan ->
            ...
            if (!state.paused && plan.enabled && permitted(context)) {
                ...
                results[plan.id] = when { state.paused -> "已暂停新执行"; !plan.enabled -> "已停用"; else -> "等待闹钟与提醒权限" }
            }
        }
