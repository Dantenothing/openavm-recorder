...
V4 release builds: vehicle `4.0.0 (57)` and phone `4.0.0 (29)`. Both apps now use the same icon. The phone opens videos directly into playback, and vehicle settings keep diagnostics and USB tools under hidden developer options. The owner has retested the RC2 USB fallback fix and reports no extra internal clip. See [V4 release notes](GITHUB_RELEASE_V4.md).
...
> [!CAUTION]
> `V4 / 4.0.0` is based on targeted owner testing on a compatible Zeekr App Lab vehicle. Broader vehicle and firmware compatibility has not been established.
>
> On the owner's single test vehicle, 2×/5×/10× time-lapse, automatic lock/vehicle-away stop, USB one-minute recording, USB playback, selected-segment phone transfer, USB removal/reinsert recovery, manifest-owned deletion and the dual-folder sound relay have passed targeted practical runs.
>
> This is not multi-vehicle, multi-day or worst-case acceptance evidence. Never operate the app while driving, and keep factory `/SentryMode/` content read-only. The owner accepted the RC2 follow-up fix; this does not establish broader fleet compatibility.
...
The `V4 / 4.0.0` release provides:

- six offline UI options on both apps: English, Simplified Chinese, Traditional Chinese, Thai, Vietnamese and Arabic;
- correctly proportioned phone playback of exported square single-camera videos;
- a simpler vehicle Settings page, with JSON diagnostics available under hidden developer tools;
...
- USB-preferred one-minute normal recording with a one-time internal fallback when the selected USB becomes unavailable;
- experimental 2×–150× time-lapse recording;
- protected/saved events that automatic cleanup does not remove;
- a thumbnail-based library covering internal, OpenAVM USB and read-only factory Sentry media;
- multi-select internal/owned-USB cleanup while factory `/SentryMode/` remains excluded;
- source-aware playback, one-minute segment selection, seeking, Session navigation and single-view zoom;
- optional display-only fisheye correction;
- automatic cleanup of manifest-proven OpenAVM USB segments under a configurable 15/30/45 GiB or custom quota;
- selected-session or selected-segment transfer over a trusted local hotspot/LAN;
- Companion playback/export of transferred media and 2560×2560 factory Sentry media;
- Companion lock/unlock sound editing plus a targeted, durable phone-to-vehicle USB relay; and
- a vehicle-side sound editor for downloaded music/video, plus listing, previewing and deleting exact WAV copies in the two supported USB sound folders.

V4 excludes the self-developed parking Sentry capture and AI model. Factory Sentry videos remain supported. Research is retained in [the archive index](research/sentry/README_V4_ARCHIVE.md); continuous parking capture is not a V4 feature.

Pairing, whole/selected-segment transfer and interrupted/offline recovery have been exercised on the owner's devices. Keep the hotspot active and preferably keep OpenAVM Companion in the foreground during a transfer. Every background state and phone vendor has not been validated.
...
The V4 implementation can record one-minute segments directly to the tested removable MediaStore volume through a file descriptor. A new Session prefers the selected writable USB; after a storage failure it falls back to internal storage at most once and does not silently reacquire USB in the same Session.

The OpenAVM USB quota defaults to `30 GiB`, with `15/30/45 GiB` presets and a custom value. Automatic quota cleanup and owned-USB deletion apply only to freshly verified OpenAVM manifests under `Download/OpenAVM`. Factory `/SentryMode/`, unknown files and incomplete/unclassified items are never quota-cleaned by OpenAVM.

This reduces routine writes to the head unit's internal flash, but moves sustained recording writes to the removable drive. USB endurance, sudden-removal behaviour, nearly-full-media behaviour and multi-day rotation therefore remain engineering risks.
...
Quota cleanup limits retained OpenAVM data, but it does **not** eliminate cumulative writes to the active storage medium.
...
For this reason, **internal fallback endurance, removable-drive endurance and sustained storage I/O remain unresolved project risks**.
...
2. Choose the latest signed release. A tag marked `Pre-release` is intended only for informed voluntary testers; use `V4 / 4.0.0` for this release.
...
The V4 release build uses the developer’s local signing configuration (`debug` signing config) to retain upgrade compatibility with previous owner builds. It produces `app-release.apk` with debugging disabled. Another developer’s local key is different and cannot update an existing owner-signed installation. Signing keys are not included in this repository.
...
- The original public baseline had approximately four hours of road testing as of 12 August 2026; later targeted private runs are recorded by feature rather than as a reliable cumulative-hour total.
...
- Direct USB recording and reinsert recovery work on the tested drive, but full-disk, repeated hard-unplug and multi-day quota-rotation matrices remain incomplete.
- Internal fallback and long-term internal/removable storage endurance have not been established.
- Companion pairing, recording transfer, selected-segment transfer, core playback/export and factory Sentry reading have received practical testing only on the owner's device set.
- Lock/unlock sound creation, targeted dual-folder USB relay and the vehicle sound toolbox work on the owner's device set. Zeekr may not refresh its custom-sound list until the vehicle UI/session is restarted, and OpenAVM cannot identify which sound is currently selected by the vehicle.
- Multi-select deletion and manifest-owned OpenAVM USB deletion passed targeted tests on the owner's drive; factory `/SentryMode/` content remains read-only and excluded from cleanup.
- Completed transfer notifications are explicitly removed. The owner reports the RC2 extra-clip fix passed retesting; V4 retains that recording code.
- Time-lapse 2×/5×/10× and lock-stop passed targeted runs, but 2×–150× coverage and long-duration acceptance remain incomplete.
...
- One later intensive playback/switching run produced an approximately ten-second camera reacquisition delay that recovered without restart; it remains a monitor item unless repeatable, longer than twenty seconds or recording-affecting.
